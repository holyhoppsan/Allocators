...
namespace PointerMath
{
	inline void* alignForward(const void* Address, size_t Alignment)
	{
		...
		const size_t AddressPlusAlignment = AddressTyped + static_cast<size_t>(Alignment - 1);
		const size_t NotAlignment = static_cast<size_t>(~(Alignment - 1));

		std::cout << "Alignment typed:             " << std::bitset<32>(AddressTyped) << std::endl;
		std::cout << "Alignment typed + alignment: " << std::bitset<32>(AddressPlusAlignment) << std::endl;
		std::cout << "Alignment Before:            " << std::bitset<32>(Alignment - 1) << std::endl;
		std::cout << "Not Alignment After:         " << std::bitset<32>(NotAlignment) << std::endl;
		std::cout << "Masked Result:               " << std::bitset<32>(AddressPlusAlignment & NotAlignment);
